...
function draw6() {
    //console.log( 'draw6' );
    drawCardsFromZone({
        cards: S[you].deck.order.splice(0, 6),
        owner: you,
        zona: 'deck'
    })
}
...
function draw3() {
    //console.log( 'draw6' );
    drawCardsFromZone({
        cards: S[you].deck.order.splice(0, 3),
        owner: you,
        zona: 'deck'
    })
}
...
function draw1() {
    //console.log( 'draw6' );
    drawCardsFromZone({
        cards: S[you].deck.order.splice(0, 1),
        owner: you,
        zona: 'deck'
    })
}
...
function btnNextPhase() {
    if (Can.pressNextBtn({
        ...
        socket.emit('pressNextBtn', {
            u: Client,
            transferInitiative: G.transferInitiative
        })
...
function emptyClick() {
    ...
    if (Stadies.current == 'block' && G.selectedTeam != null && G.selectedTeam.owner == you) {
        if (G.selectedTeam.zona == 'block') {
            moveOutBlockJSON(G.selectedTeam.card);
            G.selectedTeam.card.select(false);
            G.selectedTeam = null;
    ...
    } else if (S.phase == 'organisation' && G.selectedCard != null && Can.removeFromTeam({
        S: S,
        Known: Known,
...
function moveCardToZone(card, o) {
    ...
    Log(0, 'o.zona', o.zona);
    if (o.zona == 'deck' || o.zona == 'discard' || o.zona == 'chackra' || o.zona == 'hand') {
    ...
    } else if (o.zona == 'village' || o.zona == 'attack' || o.zona == 'block') {
...
function moveCardToZoneJSON(card, o) {
    ...
    if (card.params.zona == 'village' || card.params.zona == 'block' || card.params.zona == 'attack') {
        ...
        } else if (o.zona == 'village' || o.zona == 'block' || o.zona == 'attack') {

            removeSelfFromTeamJSON(card)

            S[o.owner][o.zona].order.push(o.team);
            S[o.owner][o.zona].team[o.team] = {
                leader: {
                    order: [card.id]
                },
                support: {
    ...
    } else if (card.params.zona == 'deck' || card.params.zona == 'discard' || card.params.zona == 'chackra' || card.params.zona == 'hand') {
        if (o.zona == 'deck' || o.zona == 'discard' || o.zona == 'chackra' || o.zona == 'hand') {
...
function drawCardsFromZone(o) {
    ...
    for (var i in o.cards) {
        ...
        moveCardToZoneJSON(window[id], {
            owner: window[id].params.owner,
            zona: 'hand'
        })
...
function organisationMove(card) {
    ...
    if (card.params.owner != you || card.params.zona != 'village' || card.params.type != 'N' || S.activePlayer != you) {
        Log(-1, 'organisationMove');
        return false;
    }
    ...
    if (!G.selectedCard) {
        ...
        if (card.params.teamPosition == 'leader' && card.params.team != G.selectedCard.params.team) {
            var obj = {
                    S: S,
                    Known: Known,
                    Accordance: Accordance,
...
function updPosition(position, sqr) {
    ...
    if (isFight) {
        ...
        for (var attackTeam in S.battlefield) {
            ...
            if (blockTeams[S.battlefield[attackTeam]] && attackTeams[attackTeam].S < blockTeams[S.battlefield[attackTeam]].S) {
